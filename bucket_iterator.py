...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        for item in batch_data:
            ...
            polarity, dependency_graph, aspect_graph, enhance_graph, fusion_graph = \
                item['context'], item['aspect'], \
                item['text_indices'], item['context_indices'], item['aspect_indices'], \
                item['left_indices'], item['polarity'], item['dependency_graph'], item['aspect_graph'], \
                item['enhance_graph'], item['fusion_graph']
            ...
            batch_aspect_graph.append(numpy.pad(aspect_graph, \
                ((0,max_len-len(text_indices)),(0,max_len-len(text_indices))), 'constant'))
            batch_enhance_graph.append(numpy.pad(enhance_graph, \
                ((0,max_len-len(text_indices)),(0,max_len-len(text_indices))), 'constant'))
            batch_fusion_graph.append(numpy.pad(fusion_graph, \
                ((0,max_len-len(text_indices)),(0,max_len-len(text_indices))), 'constant'))
        ...
        return { \
                ...
                'enhance_graph': torch.tensor(batch_enhance_graph), \
                'fusion_graph': torch.tensor(batch_fusion_graph),
            }
