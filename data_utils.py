...
class ABSADatesetReader:
    ...
    def __read_data__(fname, tokenizer):
        ...
        fin.close()
        fin = open(fname + '.graph_eh', 'rb')
        idx2gragh_e = pickle.load(fin)
        fin.close()
        fin = open(fname + '.graph_if', 'rb')
        idx2gragh_f = pickle.load(fin)
        fin.close()
        ...
        for i in range(len(lines)):
            ...
            for aspect, polarity in zip(aspect_list, polarity_list):
                ...
                enhance_graph = idx2gragh_e[graph_id]
                fusion_graph = idx2gragh_f[graph_id]
    ...
    def __init__(self, dataset='rest14', embed_dim=300):
        ...
        fname = {
            'rest14': {
                'train': './datasets/rest14_train.raw',
                'test': './datasets/rest14_test.raw'
            },
            'lap14': {
                'train': './datasets/lap14_train.raw',
                'test': './datasets/lap14_test.raw'
            },
            'rest15': {
                'train': './datasets/rest15_train.raw',
                'test': './datasets/rest15_test.raw'
            },
            'rest16': {
                'train': './datasets/rest16_train.raw',
                'test': './datasets/rest16_test.raw'
            },
            'twitter': {
                'train': './datasets/twitter_train.raw',
                'test': './datasets/twitter_test.raw'
            },
